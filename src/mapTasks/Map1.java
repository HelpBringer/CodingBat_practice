...
/*    Given a map of food keys and their topping values, modify and return the map as follows: if the key "ice cream" has a value, set that as the value for
    the key "yogurt" also. If the key "spinach" has a value, change that value to "nuts".
...
/*    Modify and return the given map as follows: if exactly one of the keys "a" or "b" has a value in the map (but not both), set the other to have that
    ...
    public Map<String, String> mapAB3(Map<String, String> map) {
        Boolean tmp=map.containsKey("a");
        if(tmp != map.containsKey("b") ){
            if(tmp){
                map.put("b",map.get("a"));
            }
            else{
